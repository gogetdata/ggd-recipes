...
* Once you have an account you will need to fork the `ggd-recipes <https://github.com/gogetdata/ggd-recipes>`_ 
  github repo. This repository is where the ggd data recipes are collected.
    ...
    * **ggd-recipes** will now be a repository attached to your acount. (github.com/<github username>/ggd-recipes) 

* You will then need to clone the forked ggd-recipes repo to the machine that you are working on. 

    * On you machine navigate to a directory were you want to donwload the forked ggd-recipes repo. 
...
* clone the forked repository to the current location on your machine
    ...
    * :code:`<USERNAME>` is your github account username. (This is where the ggd-recipes repo was forked to)
...
* Add the original ggd-recieps repo as an upstream remote repo to the one just added to your machine. This
  will make it more easy to update your forked repo with the uptream origional one.
