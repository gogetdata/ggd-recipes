...
The following setps outline how to create, check, and add a ggd data recipe. 
...
The follwoing will outline steps used to create the hg19-gaps ggd data recipe
...
* Next, identify if you need a genome coordinance file. Many of these are hosted on the ggd-recipes repo. 
  If the coordinance file is not available you can either add one to the ggd-recipes repo or ask a member of the 
  ggd team to add one. 

* Example: hg19 genome build coordinance file:
...
* Next, identify what format you want the final files to be in and what processing needs to be done
    * Example: 
        1. need to decompress a gziped file
...
----------------------------------------
The go get data (ggd) command line interface (cli) contains tools to create and test a data recipe
...
Eample:
    ...
    it into a ggd recipe:
        ...
        ::
            ...
            $ ggd from-bash -s Homo_sapiens -g hg19 --author mjc --ggd_version 1 --data_version 27-Apr-2009 --summary 'Assembly gaps from USCS' -k gaps -k region gaps hg19_data_recipe.sh

    The :code:`ggd from-bash` tools transforms the bash script you created into a data recipe. Running the above code will create 
...
--------------------------------------------
Now that you have create a ggd data recipe you need to test it to make sure it not only extracts and processes the data, but 
...
The recipes file convention is as follows:
    ...
    * The recipes directory has the following format:
        ...
        * :code:`recipes` is **recipes** directory
        ...
        * :code:`<species>` is the species that data is for. For the hg19-gaps example this would be **Homo_sapiens**
        * :code:`<genome-build>` is the genome build for the datat. For the hg19-gaps example this would be **hg19**
...
Use the following commands:
    ...
    * Add the recipe to the git repo:
        :code:`git add /recipes/genomics/Homo_sapiens/hg19/hg19-gaps/`
...
You have now create a pull request with your new data recipe. The recipe will go through a continous integration 
