...
if [[ -z $(conda info --envs | grep "*" | grep -o "\/.*") ]]; then
    ...
    env_dir=$CONDA_ROOT
    export RECIPE_DIR=$CONDA_ROOT/share/ggd/Danio_rerio/GRCz11/grcz11-reference-genome-ensembl-v1/1
    ...
    export RECIPE_DIR=$env_dir/share/ggd/Danio_rerio/GRCz11/grcz11-reference-genome-ensembl-v1/1
fi
...
for f in *; do
    ext="${f#*.}"
    filename="{f%%.*}"
    if [[ ! -f "grcz11-reference-genome-ensembl-v1.$ext" ]]  
    then
        (mv $f "grcz11-reference-genome-ensembl-v1.$ext")
    fi  
done
...
then
    recipe_env_file_name="ggd_grcz11-reference-genome-ensembl-v1_file"
...
#### Dir
recipe_env_dir_name="ggd_grcz11-reference-genome-ensembl-v1_dir"
