...
if [[ -z $(conda info --envs | grep "*" | grep -o "\/.*") ]]; then
    ...
    env_dir=$CONDA_ROOT
    export RECIPE_DIR=$CONDA_ROOT/share/ggd/Mus_musculus/mm10/mm10-reference-ucsc-v1/1
    ...
    export RECIPE_DIR=$env_dir/share/ggd/Mus_musculus/mm10/mm10-reference-ucsc-v1/1
fi
...
for f in *; do
    ext="${f#*.}"
    filename="{f%%.*}"
    if [[ ! -f "mm10-reference-ucsc-v1.$ext" ]]  
    then
        (mv $f "mm10-reference-ucsc-v1.$ext")
    fi  
done
...
then
    recipe_env_file_name="ggd_mm10-reference-ucsc-v1_file"
...
#### Dir
recipe_env_dir_name="ggd_mm10-reference-ucsc-v1_dir"
