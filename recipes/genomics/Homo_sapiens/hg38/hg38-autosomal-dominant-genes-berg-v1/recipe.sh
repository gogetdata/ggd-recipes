...
gsort unflattened_ad_genes.bed $genome \
    ...
    | bgzip -c > hg38-autosomal-dominant-genes-berg-v1.bed.gz 
tabix hg38-autosomal-dominant-genes-berg-v1.bed.gz

wget --quiet $genome2

## Get ad gene complement coordinates 
sed "1d" hg38.genome \
    | bedtools complement -i <(zgrep -v "#" hg38-autosomal-dominant-genes-berg-v1.bed.gz) -g /dev/stdin \
    | gsort /dev/stdin $genome2 \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\tstart\tend"} {print $1,$2,$3}' \
    | bgzip -c > hg38-autosomal-dominant-genes-berg-v1.compliment.bed.gz 
tabix hg38-autosomal-dominant-genes-berg-v1.compliment.bed.gz 
