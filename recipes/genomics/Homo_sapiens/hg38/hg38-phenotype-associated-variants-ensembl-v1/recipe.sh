...
## Get the phenotype associated  variant file, sort it, bgzip it, tabix it
...
bcftools reheader --fai $fai_file homo_sapiens_phenotype_associated.vcf.gz \
    | gzip -dc \
    | vt decompose -s - \
    | vt normalize -r $ref_genome -n - -o grch38-decomposed-normalized-phenotype-assocaitred.vcf 
...
bcftools reheader --fai $fai_file grch38-decomposed-normalized-phenotype-assocaitred.vcf > hg38-decomposed-normalized-phenotype-associated.vcf

rm grch38-decomposed-normalized-phenotype-assocaitred.vcf

gsort --chromosomemappings $chrom_mapping hg38-decomposed-normalized-phenotype-associated.vcf $genome \
    | bgzip -c > hg38-phenotype-associated-variants-ensembl-v1.vcf.gz

tabix hg38-phenotype-associated-variants-ensembl-v1.vcf.gz


rm hg38-decomposed-normalized-phenotype-associated.vcf
