...
cat <(grep "^#" GCA_000001405.28_GRCh38.p13_assembly_report.txt | tail -n 1) <(grep -v "^#" GCA_000001405.28_GRCh38.p13_assembly_report.txt) \
    | sed 's/ //g' \
    | bioawk -tc header 'NR>1 {print $GenBank_Accn,$UCSC_style_name,$Sequence_Length}' \
    ...
    | sed -e "s/J01415.2/MT/g" \
    | sed -e "s/na//g" > chromlength.txt
...
cat <(grep -E "^[1-9]+|^[X]|^[Y]|^(MT)" chromlength.txt | sort -k1,1) \
    ...
    | awk 'BEGIN {print "#chrom\tlength"} { if ( length($2) > 1 ) print $2"\t"$3}' \
    | sed "s/\r//g" > hg38.genome  
