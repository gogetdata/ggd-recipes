...
wget --quiet -O - https://storage.googleapis.com/gnomad-public/legacy/exac_browser/forweb_cleaned_exac_r03_march16_z_data_pLI_CNV-final.txt.gz \
    | gzip -dc \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\ttxStart\ttxEnd\tgene_symbol\ttranscript_id\texon_number\tn_bp\tmissense_z_score"} 
                             { if (NR > 1) print $3,$5,$6,$2,$1,$4,$7,$18}' \
    | gsort --chromosomemappings $chr_mapping /dev/stdin $genome \
    | uniq \
    | bgzip -c > hg19-missense-z-exac-v1.bed.gz

tabix hg19-missense-z-exac-v1.bed.gz
