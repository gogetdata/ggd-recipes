...
gsort unflattened_ar_genes.bed $genome \
    ...
    | bgzip -c > hg19-autosomal-recessive-genes-berg-blekhman-v1.bed.gz 
tabix hg19-autosomal-recessive-genes-berg-blekhman-v1.bed.gz 

wget --quiet $genome2

## Get ar gene complement coordinates 
sed "1d" hg19.genome \
    | bedtools complement -i <(zgrep -v "#" hg19-autosomal-recessive-genes-berg-blekhman-v1.bed.gz) -g /dev/stdin \
    | gsort /dev/stdin $genome2 \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\tstart\tend"} {print $1,$2,$3}' \
    | bgzip -c > hg19-autosomal-recessive-genes-berg-blekhman-v1.compliment.bed.gz 
tabix hg19-autosomal-recessive-genes-berg-blekhman-v1.compliment.bed.gz 
