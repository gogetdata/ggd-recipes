...
cat <(grep "^#" GCA_000001405.14_GRCh37.p13_assembly_report.txt | tail -n 1) <(grep -v "^#" GCA_000001405.14_GRCh37.p13_assembly_report.txt) \
    | sed 's/ //g' \
    | bioawk -tc header 'NR>1 {print $GenBank_Accn,$RefSeq_Accn,$UCSC_style_name}' \
...
## Add normal chromosome names to the list
cat <(grep -E "^[1-9]+|^[X]|^[Y]|^(MT)" chrommapping.txt | sort -k1,1) \
    | awk -v OFS="\t" '{ print $1,$1,$3}' >> chrommapping.txt

## Save the report file with a header
cat <(grep -E "^[1-9]+|^[X]|^[Y]|^(MT)" chrommapping.txt | sort -k1,1) \
    <(grep -E -v "^[1-9]+|^[X]|^[Y]|^(MT)" chrommapping.txt | sort -k2,2) \
    | sed "s/\r//g" \
    | awk 'BEGIN {print "#RefSeq_id\tUCSC_id"} {if ( $3 != "na" ) { print $2"\t"$3}}' \
    | awk -v OFS="\t" '{ if ( $1 != "na") print $0 }' > hg19_chrom_mapping_refseq2ucsc_ncbi.txt  
