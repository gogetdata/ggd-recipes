...
if [[ -z $(conda info --envs | grep "*" | grep -o "\/.*") ]]; then
    ...
    env_dir=$CONDA_ROOT
    export RECIPE_DIR=$CONDA_ROOT/share/ggd/Homo_sapiens/hg19/hg19-gene-only-features-ensembl-v1/1
    ...
    export RECIPE_DIR=$env_dir/share/ggd/Homo_sapiens/hg19/hg19-gene-only-features-ensembl-v1/1
fi
...
for f in *; do
    ext="${f#*.}"
    filename="{f%%.*}"
    if [[ ! -f "hg19-gene-only-features-ensembl-v1.$ext" ]]  
    then
        (mv $f "hg19-gene-only-features-ensembl-v1.$ext")
    fi  
done
...
then
    recipe_env_file_name="ggd_hg19-gene-only-features-ensembl-v1_file"
...
#### Dir
recipe_env_dir_name="ggd_hg19-gene-only-features-ensembl-v1_dir"
