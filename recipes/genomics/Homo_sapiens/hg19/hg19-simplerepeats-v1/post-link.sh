...
if [[ -z $(conda info --envs | grep "*" | grep -o "\/.*") ]]; then
    ...
    env_dir=$CONDA_ROOT
    export RECIPE_DIR=$CONDA_ROOT/share/ggd/Homo_sapiens/hg19/hg19-simplerepeats-v1/1
    ...
    export RECIPE_DIR=$env_dir/share/ggd/Homo_sapiens/hg19/hg19-simplerepeats-v1/1
fi
...
recipe_env_name="ggd_hg19-simplerepeats-v1"
...
for f in *; do
    ext="${f#*.}"
    filename="{f%%.*}"
    (mv $f "hg19-simplerepeats-v1.$ext")
done
