...
python parse_gtf_by_gene.py $grch38_gtf berg_ad.tsv unflattened_ad_genes.bed  
...
gsort unflattened_ad_genes.bed $genome \
    ...
    | bgzip -c > grch38-autosomal-dominant-genes-berg-v1.bed.gz 
tabix grch38-autosomal-dominant-genes-berg-v1.bed.gz
...
sed "1d" GRCh38.genome \
    | bedtools complement -i <(zgrep -v "#" grch38-autosomal-dominant-genes-berg-v1.bed.gz) -g /dev/stdin \
    | gsort /dev/stdin $genome \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\tstart\tend"} {print $1,$2,$3}' \
    | bgzip -c > grch38-autosomal-dominant-genes-berg-v1.compliment.bed.gz 
tabix grch38-autosomal-dominant-genes-berg-v1.compliment.bed.gz 
