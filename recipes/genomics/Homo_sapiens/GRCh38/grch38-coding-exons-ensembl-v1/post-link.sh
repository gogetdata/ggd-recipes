...
if [[ -z $(conda info --envs | grep "*" | grep -o "\/.*") ]]; then
    ...
    env_dir=$CONDA_ROOT
    export RECIPE_DIR=$CONDA_ROOT/share/ggd/Homo_sapiens/GRCh38/grch38-coding-exons-ensembl-v1/1
    ...
    export RECIPE_DIR=$env_dir/share/ggd/Homo_sapiens/GRCh38/grch38-coding-exons-ensembl-v1/1
fi
...
for f in *; do
    ext="${f#*.}"
    filename="{f%%.*}"
    if [[ ! -f "grch38-coding-exons-ensembl-v1.$ext" ]]  
    then
        (mv $f "grch38-coding-exons-ensembl-v1.$ext")
    fi  
done
...
then
    recipe_env_file_name="ggd_grch38-coding-exons-ensembl-v1_file"
...
#### Dir
recipe_env_dir_name="ggd_grch38-coding-exons-ensembl-v1_dir"
