...
wget --quiet http://hgdownload.soe.ucsc.edu/goldenPath/hg38/database/knownGene.txt.gz
...
python map_knowngeneid_2_symbol.py kgXref.txt.gz knownGene.txt.gz \
    | awk -v OFS="\t" -v FS="\t" 'BEGIN {print "#Table Info: https://genome.ucsc.edu/cgi-bin/hgTables?db=hg38&hgta_group=genes&hgta_track=knownGene&hgta_table=knownGene&hgta_doSchema=describe+table+schema\n#chrom\ttxStart\ttxEnd\tstrand\tknowngene_ID\tgene_symbol\tcdsStart\tcdsEnd\texonCount\texonStarts\texonEnds\tproteinID\tdescription"} 
                             {print $4,$6,$7,$5,$3,$1,$8,$9,$10,$11,$12,$13,$2}' \
    | gsort --chromosomemappings $chrom_mapping /dev/stdin $genome \
    | bgzip -c > grch38-knowngenes-ucsc-v1.bed.gz

## Tabix file
tabix grch38-knowngenes-ucsc-v1.bed.gz
