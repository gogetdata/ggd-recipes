...
cat $chrom_mapping | awk '{ if (system("grep -Fq " $2 " GRCh38.genome") == 0) print $0}' > grch38-chrom-mapping-filtered.txt
...
wget --quiet -O - http://hgdownload.cse.ucsc.edu/goldenpath/hg38/database/cpgIslandExt.txt.gz \
    | gzip -dc \
    | awk '{ if (system("grep -Fq " $2 " grch38-chrom-mapping-filtered.txt") == 0)  print $0}' \
    | cut -f 2- \
    ...
    | gsort --chromosomemappings grch38-chrom-mapping-filtered.txt /dev/stdin $genome \
...
## remove extra files
rm GRCh38.genome
rm grch38-chrom-mapping-filtered.txt
