...
cat <(grep "^#" GCA_000001405.28_GRCh38.p13_assembly_report.txt | tail -n 1) <(grep -v "^#" GCA_000001405.28_GRCh38.p13_assembly_report.txt) \
    ...
    | sed -e "s/J01415.2/MT/g" > chromlength.txt

## Save the report file with a header
cat <(grep -E "^[1-9]+|^[X]|^[Y]|^(MT)" chromlength.txt | sort -k1,1) \
    <(grep -E -v "^[1-9]+|^[X]|^[Y]|^(MT)" chromlength.txt | sort -k2,2) \
    | sed "s/\r//g" \
    | awk 'BEGIN {print "#chrom\tlength"}  { print $1"\t"$2}' > GRCh38.genome
