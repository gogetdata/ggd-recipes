...
wget --quiet -O - https://storage.googleapis.com/gnomad-public/legacy/exac_browser/forweb_cleaned_exac_r03_march16_z_data_pLI_CNV-final.txt.gz \
    | gzip -dc \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\ttxStart\ttxEnd\tgene_symbol\ttranscript_id\texon_number\tnbps\tpLI"} 
                             { if (NR > 1) print $3,$5,$6,$2,$1,$4,$7,$20}' \
    | gsort /dev/stdin $genome \
    | uniq \
    | bgzip -c > pli-scores-exac-v1.bed.gz
...
python grch37_to_grch38.py $gtf_file pli-scores-exac-v1.bed.gz \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\tstart\tend\tgene\ttranscript_id\tstrand\texon_number\tpLI"}
                             {print $0} '\
    | gsort /dev/stdin $genome2 \
    | bgzip -c > grch38-pli-scores-exac-v1.bed.gz

tabix grch38-pli-scores-exac-v1.bed.gz


rm pli-scores-exac-v1.bed.gz
