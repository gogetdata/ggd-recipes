...
cat <(gzip -dc Homo_sapiens.GRCh38.100.gtf.gz | grep "^#") <(gzip -dc Homo_sapiens.GRCh38.100.gtf.gz | grep -v "^#") \
    | awk -v OFS="\t" 'BEGIN{print "#chrom\tsource\tfeature\tstart\tend\tscore\tstrand\tframe\tattribute"} { if ( $3 == "transcript") print $0}' \
    | gsort /dev/stdin $genome \
    | bgzip -c > grch38-transcript-only-features-ensembl-v1.gtf.gz

tabix grch38-transcript-only-features-ensembl-v1.gtf.gz
