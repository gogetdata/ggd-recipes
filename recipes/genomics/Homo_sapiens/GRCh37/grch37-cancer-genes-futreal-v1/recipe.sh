...
python parse_gtf_by_gene.py $grch37_gtf cancer_genes.tsv unflattened_cancer_genes.bed  
...
gsort unflattened_cancer_genes.bed $genome \
    ...
    | bgzip -c > grch37-cancer-genes-futreal-v1.bed.gz 
tabix grch37-cancer-genes-futreal-v1.bed.gz 
...
sed "1d" GRCh37.genome \
    | bedtools complement -i <(zgrep -v "#" grch37-cancer-genes-futreal-v1.bed.gz) -g /dev/stdin \
    | gsort /dev/stdin $genome \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\tstart\tend"} {print $1,$2,$3}' \
    | bgzip -c > grch37-cancer-genes-futreal-v1.compliment.bed.gz 
tabix grch37-cancer-genes-futreal-v1.compliment.bed.gz 
