...
#python parse_genes.py berg_blekhman_ad.tsv  coding_gene_file.bed unflattened_ad_genes.bed  
...
bedtools complement -i grch37-autosomal-dominant-genes-berg-blekhman-v1.bed.gz -g GRCh37.genome \
    | gsort /dev/stdin $genome \
    | sed '1d' /dev/stdin \
    | awk -v OFS="\t" 'BEGIN {print "#chrom\tstart\tend"} {print $1,$2,$3}' \
    | bgzip -c > grch37-autosomal-dominant-genes-berg-blekhman-v1.compliment.bed.gz 
tabix grch37-autosomal-dominant-genes-berg-blekhman-v1.compliment.bed.gz 
