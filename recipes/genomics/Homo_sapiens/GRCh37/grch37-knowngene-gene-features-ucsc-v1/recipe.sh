    ...
    for line in fh:
        ...
        for attribute in attributes:
            ...
            if attribute_list[0].strip() in ["gene_id","gene_name","transcript_id","exon_id"]:
                gene_symbol = kg_dict[eval(attribute_list[1].strip())] if eval(attribute_list[1].strip()) in kg_dict else "NA" 
                break
...
wget --quiet  http://hgdownload.soe.ucsc.edu/goldenPath/hg19/bigZips/genes/hg19.knownGene.gtf.gz \
...
python map_knowngeneid_2_symbol.py kgXref.txt.gz hg19.knownGene.gtf.gz \
    | awk -v OFS="\t" 'BEGIN {print "#Table Info: http://genome.ucsc.edu/cgi-bin/hgTrackUi?db=hg19&g=knownGene\n#chrom\tsource\tfeature\tstart\tend\tscore\tstrand\tframe\tattribute"} 
                             {print $0}' \
    | gsort --chromosomemappings $chrom_mapping /dev/stdin $genome \
    | bgzip -c > grch37-knowngenes-gene-features-ucsc-v1.gtf.gz

tabix grch37-knowngenes-gene-features-ucsc-v1.gtf.gz
