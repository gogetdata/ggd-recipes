...
if [[ -z $(conda info --envs | grep "*" | grep -o "\/.*") ]]; then
    ...
    env_dir=$CONDA_ROOT
    export RECIPE_DIR=$CONDA_ROOT/share/ggd/Homo_sapiens/GRCh37/grch37-reference-genome-v1/1
    ...
    export RECIPE_DIR=$env_dir/share/ggd/Homo_sapiens/GRCh37/grch37-reference-genome-v1/1
fi
...
recipe_env_name="ggd_grch37-reference-genome-v1"
...
for f in *; do
    ext="${f#*.}"
    filename="{f%%.*}"
    (mv $f "grch37-reference-genome-v1.$ext")
done
