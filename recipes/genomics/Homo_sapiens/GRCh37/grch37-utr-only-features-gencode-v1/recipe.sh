...
cat <(gzip -dc gencode.v34lift37.annotation.gtf.gz | grep "^#") <(python get_utr_features.py gencode.v34lift37.annotation.gtf.gz) \
    | gsort --chromosomemappings  $chrom_mapping /dev/stdin $genome \
    | bgzip -c > grch37-utr-only-features-gencode-v1.gtf.gz

tabix grch37-utr-only-features-gencode-v1.gtf.gz
