...
gzip -dc variation/clinvar-orig.vcf.gz | awk 'BEGIN{FS=OFS="\t"}($1 ~ /^#/) { print } ($1 == "MT"){$1="M"} ($1 !~ /^#/) { print "chr"$0 }' | gsort /dev/stdin https://raw.githubusercontent.com/gogetdata/ggd-recipes/dev/genomes/hg38/hg38.genome | bgzip -c > variation/clinvar.vcf.gz
