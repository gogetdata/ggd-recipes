#!/bin/bash
set -eo pipefail

# converted from: ../cloudbiolinux/ggd-recipes/hg38-noalt/1000g_snps.yaml

mkdir -p $PREFIX/share/ggd/Homo_sapiens/hg38-noalt/hg38-noalt-1000g_snps/ && cd $PREFIX/share/ggd/Homo_sapiens/hg38-noalt/hg38-noalt-1000g_snps/

url=https://s3.amazonaws.com/biodata/hg38_bundle
base=1000G_phase1.snps.high_confidence.b38.primary_assembly
new=1000G_phase1.snps.high_confidence
mkdir -p variation
for suffix in .vcf.gz .vcf.gz.tbi
do
  [[ -f variation/$new$suffix ]] || wget -c -O variation/$new$suffix $url/$base$suffix
done
